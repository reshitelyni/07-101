...
-- tests
test_suite1 = test_suite_expect "Empty List" (elementAtKth []) [0, 1] [Nothing, Nothing :: Maybe Bool]
test_suite2 = test_suite_expect "Out Of Bound" (elementAtKth [1]) [-1, 0, 2] [Nothing, Nothing, Nothing]
test_suite3 = test_suite_expect "Happy Path" (elementAtKth [1..5]) [1, 5] [Just 1, Just 5]

test_suites = [test_suite1, test_suite2, test_suite3]
-- wrapper
main = do
    putStrLn $ name ++ " Start"
...
-- impl
elementAtKth [] _ = Nothing
elementAtKth ls k
    | k <= 0 = Nothing
    | k > (length ls) = Nothing
elementAtKth ls k = Just $ ls !! (k-1)
